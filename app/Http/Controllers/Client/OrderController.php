...
class OrderController extends Controller
{
  ...
  public function index(Request $request)
  {
    ...
    return $this->success(
      new OrderCollectionResource($order),
      'Orders retrieved successefully.',
      200
    );
  }
  ...
  public function rate(ReviewRequest $request, Order $order)
  {
    try {
      $data = $request->validated();
      $review=$this->orderService->rateOrder($order, $data);
      return $this->success(
        new ReviewResource($review),
        "data reviwed succssefully",
        201
      );
    } catch (\Exception $e) {
